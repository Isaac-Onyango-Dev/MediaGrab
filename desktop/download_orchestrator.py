...
class DownloadOrchestrator:
    ...
    def start_analysis(self, url: str) -> None:
        ...
        self.ui_manager.set_analyze_button_state("disabled", "Analyzing…")
        self.ui_manager.update_status("info", "Analyzing…")
    ...
    def _manager(self):
        """The shared DownloadManager owned by the main window."""
    ...
    def _render_playlist_info(self, result: dict) -> None:
        ...
        ctk.CTkLabel(self.ui_manager.info_body, text=f"{result['count']} videos  ·  {result['platform'].title()}",
    ...
    def start_download(self) -> None:
        ...
        # Update UI
        self.ui_manager.set_download_button_state("disabled", " Downloading…")
        self.ui_manager.set_analyze_button_state("disabled")
        self.ui_manager.update_status("info", "Downloading…")
        ...
        from shared.yt_dlp_helper import normalize_quality
        ...
        threading.Thread(
            ...
            kwargs=dict(
                ...
                on_progress=lambda p, sp, e, f, ci=None, ti=None: self.parent.after(
                    0, self.update_progress,
                    {"task_id": task_id, "progress": p, "speed": sp, "eta": e,
                     "filename": f, "current_item": ci, "total_items": ti},
                ),
                on_complete=lambda d: self.parent.after(0, self._on_download_complete, d, task_id),
                on_error=lambda err: self.parent.after(0, self._on_download_error, err, task_id),
            ),
            daemon=True,
        ).start()
    ...
    def pause_download(self) -> bool:
        ...
        if self.download_manager and self.current_task_id:
            success = self.download_manager.pause(self.current_task_id)
            if success:
    ...
    def resume_download(self) -> bool:
        ...
        if self.download_manager and self.current_task_id:
            success = self.download_manager.resume(self.current_task_id)
            if success:
    ...
    def cancel_download(self) -> None:
        ...
        if self.download_manager and self.current_task_id:
            ...
            self.download_manager.cancel(task_id)
            self._on_download_error("Cancelled", task_id)
